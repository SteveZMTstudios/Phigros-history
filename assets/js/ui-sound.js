(function () {
  ...
  var isSoundEnabled = localStorage.getItem('ui_sound_enabled') !== 'false';
  function playSound(name) {
    ...
    sound.play().catch(function (e) {
      return console.warn('Sound play blocked:', e);
    });
  }
  ...
  function updateSoundButtons() {
    var icon = isSoundEnabled ? 'volume_up' : 'volume_off';
    document.querySelectorAll('.sound-toggle-btn .material-icons').forEach(function (el) {
      el.textContent = icon;
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('sound_tip_shown')) {
      var tip = document.getElementById('sound-tip');
      if (tip) {
        setTimeout(function () {
          tip.style.opacity = '1';
        }, 1500);
    ...
    var hideSoundTip = function hideSoundTip() {
      var tip = document.getElementById('sound-tip');
    ...
    document.body.addEventListener('click', function (e) {
      var toggleBtn = e.target.closest('.sound-toggle-btn');
      if (toggleBtn) {
        e.stopPropagation();
        toggleSound();
        hideSoundTip();
    ...
    document.addEventListener('click', function (e) {
      var target = e.target.closest('button, a, .version-card, .major-version-btn, #start-screen');
      if (!target) {
        // Empty space click
      ...
      if (target.id === 'back-btn' || target.classList.contains('close-btn-desktop') || target.classList.contains('close-btn-mobile') || target.id === 'close-announcement') {
        playSound('cancel');
        return;
      }
      ...
      if (target.classList.contains('version-card') || target.tagName === 'A' || target.tagName === 'BUTTON' || target.id === 'info-btn' || target.id === 'language-btn' || target.id === 'start-language-btn' || target.id === 'start-screen' || target.id === 'version-search') {
        playSound('click');
        return;
      }
    ...
    var scrollThreshold = 100; // ms between ticks
